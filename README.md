...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`async-stream`](https://crates.io/crates/async-stream).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `async-stream` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

`stream!` and `try_stream!` macros for async streams
...
Original work copyrighted by the upstream `async-stream` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
