 ...
 * as possible.
 *
 * Upstream `next<S: Stream + Unpin>` returns a future that polls `&mut stream`
 * exactly once. The Kotlin translation accepts any [Flow] and reads its first
 * emission. Type parameter `T` is constrained to `Any` because `firstOrNull`
 * uses `null` as the end-of-stream sentinel and would otherwise collide with
 * a legitimate `null` value.
 */
