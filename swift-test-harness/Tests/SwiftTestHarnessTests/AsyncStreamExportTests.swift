...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `AsyncStream.swiftmodule/`
...
//      `import AsyncStream` above would fail at compile time.
//
//   2. The static archive `libAsyncStream.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "AsyncStream" }` and
//      `flattenPackage = "io.github.kotlinmania.asyncstream"` configuration in
...
//      via the `AsyncStreamLibrary` product.
//
...
final class AsyncStreamExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "AsyncStream swift module imported cleanly")
    }
}
